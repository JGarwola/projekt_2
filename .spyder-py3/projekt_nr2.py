...
def w_walcu(ato):
    ...
    #z =float(ato.pozycja[2]) juz nie potrzebne
    #if(z>=r and (x**2 +y**2 <= (R-r)**2)):
    #zmienilem zeby walec tworzyl sie w obie strony bo wydaje mi sie 
    #ze wtedy latwiej o periodycznosc bedzie zadbac
    #uwzlednilem to przy sprawdzaniu h
    if( (x**2 +y**2 <= (R-r)**2) ):
        return True
    else:
        return False
...
#dodalem liczenie odleglosci w z modulo self.h_max na obrazku wygląda obiecująco
def dystans(newato,ato,odl,h):
    d2 = m.sqrt((float(newato.pozycja[0])-float(ato.pozycja[0]))**2 +
                (float(newato.pozycja[1])-float(ato.pozycja[1]))**2 + 
                (  (float(newato.pozycja[2])-float(ato.pozycja[2]))%h  )**2)
    if(d2 <= odl):
        return True
    else:
        return False
    ...
    def __init__(self, atomy=[], h_max = 2*r):
        ...
        for i in range(N):
            while True:
                ...
                if i>100:
                    last = atomy[(int(random.uniform(0, i-100)))] 
                else:
                    last = atomy[(int(random.uniform(0, i)))] 
                #wybiera losowy atom jako punkt odniesienia
                #przy czym dodalem zeby nie wybieral 100 ostatnich
                #to zapobuega tworzeniu nitek w pewnym stopniu
                ...
                if(not w_walcu(a)):
                    continue
                for at in atomy:
                    if(dystans(a,at,d_min,self.h_max)==True):
                        za_blisko = 1
                        break
            ...
            #sprawdza h w przypadku budowania walca w dwie strony  
            if (  abs(float(a.pozycja[2])-float(atomy[-1].pozycja[2])) +2*r  ) > self.h_max:
                self.h_max = float(a.pozycja[2])-float(atomy[-1].pozycja[2]) +2*r 
